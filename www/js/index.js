...
var app = {
  ...
  onDeviceReady: function() {
    ...
    $('#sendSMS').on("click", function() {
      alert('start');
      smsplugin.send($('#telephone').val(), $('#smsMessage').val(), '', function(result) {
        alert('ok');
      }, function(error) {
        alert('error');
      });
...
$('#getSMS').on("click", function() {
  ...
  smsplugin.startReception(getSMSResult, function(error) {
    alert('error ' + error);
  });
  alert('end get sms');
});
...
$('#stopGetSMS').on("click", function() {
  smsplugin.stopReception(function(result) {
    alert('stop get sms!' + result);
  }, function(error) {
    alert('error ' + error);
  });
});
...
//  navigator.geolocation.getCurrentPosition (
  watchPiditionID = navigator.geolocation.watchPosition(
          geolocationSuccess,
          function() {
            alert('Error Get My Location')
          },
          {maximumAge: 20000, timeout: 10000}
  );
}

function getMyPositionNow() {
  mapViewIsSet = false;
  navigator.geolocation.clearWatch(watchPiditionID);
  getMyPosition();
}
...
function geolocationSuccess(position) {
  ...
  $('#info-latinude').html(position.coords.latitude);
  $('#info-longtinude').html(position.coords.longitude);
  $('#info-accuracy').html(position.coords.accuracy);
  $('#info-timestamp').html(position.timestamp);
  $('#info-speed').html(position.coords.speed);

  if (mapViewIsSet === false) {
